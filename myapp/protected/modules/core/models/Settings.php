...
class Settings extends YModel
{
    ...
    */
    ...
    {
        return [
            'id'            => Yii::t('CoreModule.core', 'ID'),
            'module_id'     => Yii::t('CoreModule.core', 'Module'),
            'param_name'    => Yii::t('CoreModule.core', 'Parameter name'),
            'param_value'   => Yii::t('CoreModule.core', 'Parameter value'),
            'create_time' => Yii::t('CoreModule.core', 'Creation date'),
            'update_time'   => Yii::t('CoreModule.core', 'Change date'),
            'user_id'       => Yii::t('CoreModule.core', 'User'),
        ];
    }
    ...
    public static function fetchModuleSettings($moduleId, array $params = null)
    {
        ...
        if ($moduleId) {
            ...
            $dependency = new TagsCache($moduleId, 'core');

            $q = Settings::model()->cache(Yii::app()->getModule('core')->coreCacheTime, $dependency)->findAll(
                $criteria
            );
    ...
    public static function saveModuleSettings($moduleId, $paramValues)
    {
        ...
        foreach ($paramValues as $name => $value) {
           ...
           $setting = Settings::model()->find(
                'module_id = :module_id and param_name = :param_name', //module_id = :module_id and
                [':module_id' => $moduleId, ':param_name' => $name] //':module_id' => $moduleId,
            );
