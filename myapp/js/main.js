...
function leftMenu()
{
    var ob = $('.left_menu_hiddon');
    // Click на ссылгу
    ob.children('.left_menu').children('.home_menu').children('li').children('a').click(function(){
        var objects = $(this);
        var id_is = objects.parent('li').attr('id');
        var id_i = id_is.split('-');

        openMenu(objects,id_i[1]);
    });
    // Click на иконку
    ob.children('.right_in_menu').children('a').click(function(){
        var time;
        var itemss = $(this);
        var item_ress = itemss.attr('id');
        var item_on = item_ress.split('-');
        var objects = itemss.parent('.right_in_menu').parent('.left_menu_hiddon').children('.left_menu').children('.home_menu').children('li').children('a');
        var $is, closeRight;
        objects.parent('li').find('.child_col').each(function(){
            var closed = $(this);
            var rew = closed.css('left');
            if(rew == '0px'){
                 $is = closed.parent('li').attr('id');
                var isw = $is.split('-');
                var is = objects.parent('li#item-'+isw[1]).find('.child_col').children('a');
                closeRight = objects.parent('li#item-'+isw[1]).parent('.home_menu').parent('.left_menu').parent('.left_menu_hiddon');
                clo(closeRight);
                cloLeft(is);
            }
        });
        ...
        time = setTimeout(function () {
            openMenu(objects,item_on[1]);
        }, 800);
    });

}
// Открытия подменю
function openMenu(objects,id){
    objects.parent('li#item-'+id).find('.child_col').animate({
        "left": "0"
    }, {queue:false, duration: 800, easing:"easeInOutQuad", complete:function(){
        var objec = $(this);
        closeMenu(objec);
...
// Закрытия через назад
function closeMenu(objects){
	var $close;
    $close = objects.parent('li').parent('.home_menu').parent('.left_menu').parent('.left_menu_hiddon');

    var ids = objects.parent('li').attr('id');
    var $ids_in = ids.split('-');
    $close.children('.right_in_menu').children('a').each(function(){
        var items = $(this);

        var item_res = items.attr('id');
        var item_on = item_res.split('-');

        if(item_on[1] == $ids_in[1]){
            $('.right_in_menu a').removeClass('hover');
    ...
    $close.animate({
        "width": "298px"
    }, {queue:false, duration: 200, easing:"easeInOutQuad", complete:function(){ }});

    objects.children('a').click(function(){
...
// Закрытия правой блок с иконками
function clo(close){
    close.animate({
        "width": "258px"
    }, {queue:false, duration: 200, easing:"easeInOutQuad", complete:function(){
...
// Закрытия подменю
function cloLeft(prevs){
    prevs.parent('.child_col').animate({
        "left": "-258px"
    }, {queue:false, duration: 800, easing:"easeInOutQuad", complete:function(){
