<?php
/**
 * WebModule - базовый класс для всех модулей
 *
 * Все модули, разработанные для должны наследовать этот класс.
 * Панель управления, собираемая из модулей, использует определенные методы этого базового класса.
 *
 * @category CoreModule
...
abstract class WebModule extends CWebModule
{
    ...
    /**
     * @var int некоторые компоненты Websum автоматически кэширует, если время жизни кэша не указано - берется это значение
     */
    ...
    /**
     * Путь к ресурсам модуля, например application.modules.core.views.assets
     * @var string
     */
    public $assetsPath;
    private $_assetsUrl;
    ...
    {
        return Yii::t('CoreModule.core', 'Core community!');
    }
     ...
     * {
     *       return array(
     *           Yii::t('CoreModule.core','Blogs')     => '/blog/blogBackend/admin/',
     *           Yii::t('CoreModule.core','Posts')    => '/blog/postBackend/admin/',
     *           Yii::t('CoreModule.core','Members') => '/blog/BlogToUserBackend/admin/',
     *      );
     * }
     *
     */
     ...
     *            'message' => Yii::t('CoreModule.core', 'Please, set images uploading directory! {link}', array(
     *                '{link}' => CHtml::link(Yii::t('CoreModule.core', 'Change module settings'), array('/core/backend/modulesettings/', 'module' => $this->id))
     *            ))
     *        );
    ...
    {
        return [
            'adminMenuOrder' => Yii::t('CoreModule.core', 'Menu items order'),
            'coreCacheTime'  => Yii::t('CoreModule.core', 'Cache time')
        ];
    ...
    public function getEditableParamsGroups()
    {
        return [
            'main' => [
                'label' => Yii::t('CoreModule.core', 'Main module settings'),
                'items' => [
                    'adminMenuOrder',
                    'coreCacheTime'
                ]
    ...
    {
        $modulesNoDisable = Yii::app()->getCache()->get('CoreModulesNoDisable');
        if ($modulesNoDisable === false) {
            ...
            Yii::app()->getCache()->set(
                'CoreModulesNoDisable',
                $modulesNoDisable,
                Yii::app()->getModule('core')->coreCacheTime,
                new TagsCache('core', 'installedModules')
            );
        }
    ...
    {
        $modulesDependent = Yii::app()->getCache()->get('CoreModulesDependenciesAll');
        if ($modulesDependent === false) {
            ...
            Yii::app()->getCache()->set(
                'CoreModulesDependenciesAll',
                $modulesDependent,
                0,
                new TagsCache('installedModules', 'core')
            );
        }
    ...
    {
        $modulesDependent = Yii::app()->getCache()->get('CoreModulesDependent');
        if ($modulesDependent === false) {
    ...
    /**
     * устанавливает checkbox включенным по умолчанию при установке Core
     *
     * @return bool
     *
     * @since 0.5
    ...
    {
        $modulesInstalled = Yii::app()->getCache()->get('CoreModulesInstalled');
        ...
        if ($modulesInstalled === false) {
            ...
            // Зависимость на тег:
            $chain->dependencies->add(
                new TagsCache('installedModules', 'disabledModules', 'core', $this->getId())
            );

            Yii::app()->getCache()->set(
                'CoreModulesInstalled',
                $modulesInstalled,
                Yii::app()->getModule('core')->coreCacheTime,
                $chain
            );
        ...
        $upd = Yii::app()->getCache()->get('CoreModuleUpdates_' . $this->getId());
    ...
    public function getDeActivate($noDependent = false)
    {
        ...
        if (!is_file($fileConfig) && $this->id != 'install') {
            throw new CException(Yii::t('CoreModule.core', 'Module already disabled!'));
        } else {
            ...
            if ($this->getIsNoDisable()) {
                throw new CException(Yii::t('CoreModule.core', 'This module can\'t be disabled!'));
    ...
    {
        if ($this->getIsActive()) {
            throw new CException(Yii::t('CoreModule.core', 'Disable module first!'));
        }
    ...
    public function uninstallDB()
    {
        ...
        throw new CException(Yii::t('CoreModule.core', 'Error when deleting module DB!'));
    }
    ...
    {
        return [
            self::CHOICE_YES => Yii::t('CoreModule.core', 'yes'),
            self::CHOICE_NO  => Yii::t('CoreModule.core', 'no'),
        ];
    }
    ...
    {
        return ['/core/backend/modulesettings', 'module' => $this->getId()];
    }
