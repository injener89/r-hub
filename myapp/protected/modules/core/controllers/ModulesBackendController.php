 ...
 * @since 0.8
 */
class ModulesBackendController extends core\components\controllers\BackController
{
    ...
    public function actionModuleStatus()
    {
        ...
        elseif (!isset($status) || !in_array($status, [0, 1, 2])) {
            Yii::app()->ajax->failure(Yii::t('CoreModule.core', 'Status for handler is no set!'));
        }
        ...
        if (isset($module) && !empty($module)) {
                ...
                switch ($status) {
                    case 0:
                        if ($module->getIsActive()) {
                            ...
                            $message = Yii::t('CoreModule.core', 'Module disabled successfully!');
                        } else {
                            $module->getUnInstall();
                            $message = Yii::t('CoreModule.core', 'Module uninstalled successfully!');
                        }
                        $result = true;
                        break;
                    ...
                    case 1:
                        ...
                        if ($module->getIsInstalled()) {
                            $module->getActivate();
                            $message = Yii::t('CoreModule.core', 'Module enabled successfully!');
                        } else {
                            $module->getInstall();
                            $message = Yii::t('CoreModule.core', 'Module installed successfully!');
                        }
                        $result = true;
                        break;
                    case 2:
                        $message = ($result = Yii::app()->moduleManager->updateModuleConfig($module))
                            ? Yii::t('CoreModule.core', 'Settings file "{n}" updated successfully!', $name)
                            : Yii::t(
                                'CoreModule.core',
                        ...
                        Yii::app()->getUser()->setFlash(
                            $result ? core\widgets\YFlashMessages::SUCCESS_MESSAGE : core\widgets\YFlashMessages::ERROR_MESSAGE,
                            $message
                        );
                        break;
                    default:
                        $message = Yii::t('CoreModule.core', 'Unknown action was checked!');
                        break;
                }
             ...
             **/
            Yii::app()->ajax->failure(Yii::t('CoreModule.core', 'Module was not found or it\'s enabling finished'));
        }
    }
